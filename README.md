...
As you can see, my result is mostly inline with the result coming from R package. It's worth noting that smbinning failed to calculate the WOE/IV for DAYS_LAST_PHONE_CHANGE attributes and get Inf result instead for unknown reason. Moreover, the average time it takes for the PySpark module to calcualte WOE/IV of each variable (with 307511 rows) is only 1.17s using 8 core CPUs, while for the R smbinning package it takes on average 54s to process one variable with 1 CPU! So the key advantage of this PySPark module is its high efficiency and streamlining of data preprocessing, feature engineering and statistical modeling all within parallel computing framework. 
